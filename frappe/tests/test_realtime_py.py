...
class TestConfig(unittest.TestCase):
	...
	def test_worker_threads_default(self):
		self.assertEqual(self._config().worker_threads, 64)
...
class TestServerApp(unittest.IsolatedAsyncioTestCase):
	"""health_app and RealtimeServer wiring; needs the real socketio/uvicorn deps."""
	...
	async def _request(self, scope: dict) -> list[dict]:
		sent = []
		...
		await self.server_mod.health_app(scope, receive, send)
		return sent

	async def test_health_route(self):
		sent = await self._request({"type": "http", "path": "/health"})
		self.assertEqual(sent[0]["status"], 200)
		self.assertEqual(sent[1]["body"], b"ok")

	async def test_unknown_http_path(self):
		sent = await self._request({"type": "http", "path": "/nope"})
		self.assertEqual(sent[0]["status"], 404)

	async def test_websocket_scope_is_closed_not_answered_with_http(self):
		# engineio routes non-socket.io websocket scopes here; an HTTP response would
		# make uvicorn raise on every handshake.
		sent = await self._request({"type": "websocket", "path": "/nope"})
		self.assertEqual(sent, [{"type": "websocket.close", "code": 1000}])
