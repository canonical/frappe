...
async def health_app(scope: dict, receive, send) -> None:
	"""Minimal ASGI app for non-socket.io routes. Serves GET /health.

	engineio routes websocket scopes here too; answering those with an HTTP
	response makes uvicorn raise on every handshake."""
	if scope["type"] != "http":
		await receive()
		await send({"type": "websocket.close", "code": 1000})
		return

	healthy = scope["path"] == "/health"
	status, body = (200, b"ok") if healthy else (404, b"not found")
...
class RealtimeServer:
	...
	async def _on_startup(self) -> None:
		# The interpreter default (min(32, cpu + 4)) is too small for frappe_context
		# handlers, which hold a thread for a whole DB cycle.
		asyncio.get_running_loop().set_default_executor(
			ThreadPoolExecutor(max_workers=self.config.worker_threads, thread_name_prefix="realtime-worker")
		)
		self.bridge.start()
